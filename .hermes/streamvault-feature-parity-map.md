...
| PWAPanel and push-sw | PWA and push | Install and receive alerts | PWA settings and prompt | permission, subscribe, click target | keep and improve | High | real device QA |
| WebSocketMonitor | Raw diagnostics | Debug connection | Admin Diagnostics | admin only | admin-only | Low | admin QA |
| Logger and API debug paths | Developer diagnostics | Troubleshoot | dev or admin only | hidden from normal users | capsule | Medium | source review |
