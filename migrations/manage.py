...
def list_all_migrations() -> List[Dict[str, Any]]:
    ...
    for script_path in migration_scripts:
        ...
        results.append({"name": script_name, "status": status, "path": script_path, "modified": modified})
...
def check_migration_status() -> None:
    ...
    for migration in migrations:
        ...
        reset_color = "\033[0m"
        print(
            f"{status_color}{migration['status']}{reset_color} - {migration['name']} (modified: {migration['modified']})"
        )
    ...
    with SessionLocal() as session:
        if status:
            ...
            exists = session.execute(
                text("SELECT 1 FROM migrations WHERE script_name = :name"), {"name": script_name}
            ).fetchone()
            ...
            # Mark as unapplied
            result = session.execute(text("DELETE FROM migrations WHERE script_name = :name"), {"name": script_name})
            session.commit()
...
def parse_args():
    ...
    mark_parser.add_argument(
        "--status", choices=["applied", "unapplied"], required=True, help="Status to mark the migration as"
    )
    ...
    run_parser.add_argument("--force", action="store_true", help="Force run even if already applied")
