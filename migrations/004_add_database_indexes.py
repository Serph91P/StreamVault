...
def run_migration():
    ...
    try:
        ...
        session.execute(
            text("CREATE INDEX IF NOT EXISTS idx_streams_streamer_active ON streams (streamer_id, ended_at)")
        )
        session.execute(
            text("CREATE INDEX IF NOT EXISTS idx_streams_streamer_recent ON streams (streamer_id, started_at)")
        )
        session.execute(
            text("CREATE INDEX IF NOT EXISTS idx_streams_category_recent ON streams (category_name, started_at)")
        )
        ...
        session.execute(
            text("CREATE INDEX IF NOT EXISTS idx_recordings_stream_status ON recordings (stream_id, status)")
        )
        session.execute(
            text("CREATE INDEX IF NOT EXISTS idx_recordings_status_time ON recordings (status, start_time)")
        )
        ...
        session.execute(
            text("CREATE INDEX IF NOT EXISTS idx_stream_events_stream_type ON stream_events (stream_id, event_type)")
        )
        session.execute(
            text("CREATE INDEX IF NOT EXISTS idx_stream_events_stream_time ON stream_events (stream_id, timestamp)")
        )
        session.execute(
            text("CREATE INDEX IF NOT EXISTS idx_stream_events_type_time ON stream_events (event_type, timestamp)")
        )
        logger.info("✅ Added stream_events indexes")

        # Stream metadata indexes - CRITICAL for performance (20+ queries in codebase)
        session.execute(text("CREATE INDEX IF NOT EXISTS idx_stream_metadata_stream_id ON stream_metadata (stream_id)"))
        logger.info("✅ Added stream_metadata indexes")
