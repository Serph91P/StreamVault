...
| 331-403 viewport matrix | exact config values, Chromium 17-route/22-row/both-theme collection, current Chromium/Firefox representative a11y and run-205 WebKit stage reports | each WebKit dark/light 390x844 and 1440x900 report stops at `readiness` with `locator.count` timeout 60000 ms before keyboard/tree/axe | P1 | Playwright JSON/traces | partial: Chromium full matrix, Chromium/Firefox a11y, WebKit failure evidence |
