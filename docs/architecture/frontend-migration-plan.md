...
| 238-328 phase 0 | generated JSON, six docs, gate logs, current Chromium/Firefox/WebKit artifact reports and four raw LHRs | native capture remains separate; WebKit readiness failure remains open | P0/P1 | baseline reports | captured with open defects |
| 331-403 viewport matrix | exact config values, Chromium 17-route/22-row/both-theme collection, current Chromium/Firefox representative a11y and current WebKit stage reports | WebKit source-derived readiness inspection exhausts all four representative 60-second observations before keyboard/tree/axe collection | P1 | Playwright JSON/traces | partial: Chromium full matrix, Chromium/Firefox a11y, WebKit failure evidence |
...
Browser install is not browser acceptance. The approved digest-pinned Playwright container launches Chromium, Firefox and WebKit without host-library installation; Chromium completed the retained full mock matrix and the current candidate reran representative Chromium and Firefox a11y collection. Current WebKit reports all four source-derived `.streamers-view` observations at the persisted `readiness` stage after the bounded 60-second budget, with traces retained before keyboard/tree/axe collection. Native Android/iOS/Windows/macOS evidence is a separate manual gate.
