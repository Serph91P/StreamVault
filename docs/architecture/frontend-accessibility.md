...
- The unchanged route-audit harness collected the full Chromium matrix for all 17 source-derived scenarios across required viewports and both themes. It recorded 1168 hit-test, 608 overlap and 270 undersized findings in 748 observations plus four serious or critical axe results. The current candidate reran four representative a11y/tree/keyboard/axe observations in Chromium and Firefox successfully. Its four WebKit observations persist at the `readiness` stage with raw reports and traces after the 60-second budget, so WebKit keyboard/tree/axe values remain intentionally absent. These are recorded findings, not waived acceptance.
