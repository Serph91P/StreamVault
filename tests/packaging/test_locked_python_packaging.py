...
def test_requirements_export_is_locked_uv_production_export() -> None:
    ...
    assert _non_comment_lines(REQUIREMENTS.read_text(encoding="utf-8")) == _non_comment_lines(
        completed.stdout
    )
...
def test_wheel_build_uses_metadata_without_importing_application(tmp_path: Path) -> None:
    subprocess.run(
