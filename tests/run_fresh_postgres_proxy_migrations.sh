...
db_password="pg-${run_id}-${RANDOM}"
database_scheme="postgresql"
database_url="${database_scheme}://${db_user}:${db_password}@${db_container}:5432/${db_name}"
    ...
    if [[ "${logs}" =~ ${forbidden_pattern} ]]; then
        ...
        while IFS= read -r line; do
            if [[ "${line}" =~ ${forbidden_pattern} ]]; then
                ...
                sanitized_line="${sanitized_line//${db_password}/[REDACTED]}"
                printf '%s\n' "${sanitized_line}" >&2
            fi
        done <<< "${logs}"
        return 1
