  ...
  },
  {
    ...
    fields: [
      ...
      },
      {
        ...
        help: 'Override preferred video codecs for this streamer',
        type: 'select',
        default: '',
        options: [
          { value: '', label: 'Use global default' },
          { value: 'h264', label: 'H.264 only' },
          { value: 'h265', label: 'H.265 (HEVC) only' },
          { value: 'av1', label: 'AV1 only' },
          { value: 'h264,h265', label: 'H.264 + H.265 (recommended)' },
          { value: 'h264,h265,av1', label: 'All codecs' },
        ],
        dependsOn: (state) => Boolean((state.recording as Record<string, unknown>)?.enabled),
      },
      {
        key: 'use_global_cleanup_policy',
        label: 'Use global cleanup policy',
        help: 'When enabled, this streamer follows the global recording cleanup rules',
        type: 'checkbox',
        default: true,
      },
    ],
