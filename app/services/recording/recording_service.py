...
class RecordingService:
    ...
    async def start_recording(
        ...
        logger.info(
            f"🎬 RECORDING_SERVICE_START: stream_id={stream_id}, streamer_id={streamer_id}"
        )
        result = await self.orchestrator.start_recording(
            stream_id, streamer_id, **kwargs
        )
        logger.info(f"🎬 RECORDING_SERVICE_RESULT: recording_id={result}")
        return result
    ...
    async def update_recording_status(
        ...
        await self.orchestrator.update_recording_status(
    ...
    async def enqueue_post_processing(
        ...
        return await self.orchestrator.enqueue_post_processing(
    ...
    async def find_and_validate_mp4(
        ...
        return await self.orchestrator.find_and_validate_mp4(
    ...
    async def _monitor_and_process_recording(self, recording_id: int):
        ...
        await self.orchestrator.lifecycle_manager.monitor_and_process_recording(
            recording_id
        )
    ...
    async def _enqueue_post_processing(
        ...
        await self.orchestrator.enqueue_post_processing(
            recording_id, ts_file_path, recording_data
        )
    ...
    async def _send_recording_job_update(
        ...
        await self.orchestrator.websocket_service.send_recording_job_update(
    ...
    async def _send_background_task_update(
        ...
        """Legacy method - delegate to websocket service"""
        await self.orchestrator.websocket_service.send_background_task_update(
    ...
    async def _delayed_metadata_generation(
        ...
        """Legacy method - delegate to post-processing coordinator"""
        await self.orchestrator.post_processing_coordinator.delayed_metadata_generation(
    ...
    async def _generate_stream_metadata(
        ...
        """Legacy method - delegate to post-processing coordinator"""
        return await self.orchestrator.post_processing_coordinator.generate_stream_metadata(
    ...
    def add_active_recording(
        ...
        self.orchestrator.state_manager.add_active_recording(
            recording_id, recording_data
        )
