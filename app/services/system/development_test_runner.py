...
class DevelopmentTestRunner:
    ...
    async def _test_database(self):
        ...
        try:
            with SessionLocal() as db:
                ...
                self._record_test(
                    "Streamer Table", True, f"Found {streamer_count} streamers"
                )
    ...
    async def _test_services(self):
        ...
        service_imports = [
            ...
            (
                ...
                "app.services.notifications.notification_dispatcher",
                "NotificationDispatcher",
            ),
            (
            ...
            (
                "ThumbnailService",
                "app.services.images.thumbnail_service",
                "ThumbnailService",
            ),
            # Communication services
            (
                "ModernWebPushService",
                "app.services.communication.webpush_service",
                "ModernWebPushService",
            ),
            (
                "WebSocketManager",
            ...
            try:
                ...
                if hasattr(module, class_name):
                    ...
                    self._record_test(
                        ...
                        True,
                        f"Imported and initialized from {module_path}",
                    )
                    ...
                    self._record_test(
                        f"Service: {service_name}",
                        True,
                        f"Imported instance from {module_path}",
                    )
                ...
                self._record_test(
                    f"Service: {service_name}", False, f"Import failed: {str(e)}"
                )
            except Exception as e:
                self._record_test(
                    f"Service: {service_name}",
    ...
    async def _test_notifications(self):
        ...
        try:
            ...
            try:
                ...
                self._record_test(
                    "Notification Dispatcher Init",
                    True,
                    "NotificationDispatcher initialized successfully",
                )
            except Exception as e:
                self._record_test(
                    "Notification Dispatcher Init", False, f"Dispatcher error: {str(e)}"
                )
    ...
    async def _test_image_processing(self):
        ...
        try:
            ...
            self._record_test(
                "Image Cache Directory", True, f"Cache directory: {cache_dir}"
            )

            # Test if we can create cache directories
            os.makedirs(cache_dir, exist_ok=True)
            self._record_test(
                "Image Cache Creation", True, "Cache directory created successfully"
            )
    ...
    async def _test_authentication(self):
        ...
        try:
            with SessionLocal() as db:
                ...
                if admin_count > 0:
                    self._record_test(
                        "Admin User", True, f"Found {admin_count} admin users"
                    )
                else:
                    self._record_test(
                        "Admin User", False, "No admin users found - setup required"
                    )
    ...
    async def _test_recording_system(self):
        ...
        try:
            ...
            self._record_test(
                "Recording Directory", True, f"Recording directory: {recordings_dir}"
            )

            # Import recording services to test they load properly
            self._record_test(
                "Recording Service Import",
                True,
                "Recording service imports successfully",
            )
        ...
        try:
            ...
            try:
                ...
                self._record_test(
                    ...
                    True,
                    "RecordingService initialized successfully",
                )
            except Exception as e:
                self._record_test(
                    "Recording Service Init", False, f"Failed to initialize: {str(e)}"
                )
                return
            ...
            (
                test_streamer_id,
                test_stream_id,
            ) = await self._create_test_database_entries()

            if test_streamer_id and test_stream_id:
                # Test the actual recording workflow
                await self._test_recording_service_workflow(
                    recording_service, test_stream_id, test_streamer_id
                )

                # Test recording state management
                await self._test_recording_state_management(
                    recording_service, test_stream_id
                )

                # Cleanup test data
                await self._cleanup_test_database_entries(
                    test_streamer_id, test_stream_id
                )

        except Exception as e:
            self._record_test("Recording Service Integration", False, str(e))

    async def _create_test_database_entries(
        self,
        ...
        try:
            with SessionLocal() as db:
                ...
                self._record_test(
                    ...
                    True,
                    f"Created streamer {test_streamer.id} and stream {test_stream.id}",
                )
                return test_streamer.id, test_stream.id

        except Exception as e:
            self._record_test(
                "Test Database Entries", False, f"Failed to create test data: {str(e)}"
            )
            return None, None

    async def _test_recording_service_workflow(
        self, recording_service, stream_id: int, streamer_id: int
    ):
        """Test the complete recording service workflow"""
        try:
            self._log_test_detail(
                f"Testing recording workflow: stream_id={stream_id}, streamer_id={streamer_id}"
            )
            ...
            if recording_id:
                self._record_test(
                    "Recording Start",
                    True,
                    f"Started recording with ID: {recording_id}",
                )
                ...
                try:
                    ...
                    if status:
                        ...
                        self._record_test(
                            "Recording Status Check", False, "No status returned"
                        )
                except Exception as e:
                    self._record_test(
                        "Recording Status Check",
                ...
                try:
                    stop_result = await recording_service.stop_recording(
                        recording_id, reason="test_completion"
                    )
                    if stop_result:
                        self._record_test(
                            "Recording Stop", True, "Successfully stopped recording"
                        )
                    else:
                        self._record_test(
                            "Recording Stop", False, "Failed to stop recording"
                        )
                except Exception as e:
                ...
                self._record_test(
                    "Recording Start",
                    False,
                    "Failed to start recording - no ID returned",
                )

        except Exception as e:
            self._record_test(
                "Recording Service Workflow", False, f"Workflow test failed: {str(e)}"
            )
    ...
    async def _test_recording_state_management(self, recording_service, stream_id: int):
        ...
        try:
            ...
            if isinstance(active_recordings, list):
                self._record_test(
                    "Active Recordings Check",
                    True,
                    f"Found {len(active_recordings)} active recordings",
                )
            else:
                self._record_test(
                    "Active Recordings Check",
                    False,
                    "Failed to get active recordings list",
                )

            # Test get_stream_recordings
            try:
                stream_recordings = await recording_service.get_stream_recordings(
                    stream_id
                )
                if isinstance(stream_recordings, list):
                    self._record_test(
                        "Stream Recordings Check",
                        True,
                        f"Found {len(stream_recordings)} recordings for stream",
                    )
                else:
                    self._record_test(
                        "Stream Recordings Check",
                        False,
                        "Failed to get stream recordings",
                    )
            except Exception as e:
    ...
    async def _cleanup_test_database_entries(self, streamer_id: int, stream_id: int):
        ...
        try:
            with SessionLocal() as db:
                ...
                recordings = (
                    db.query(Recording).filter(Recording.stream_id == stream_id).all()
                )
                ...
                db.commit()
                self._record_test(
                    "Test Data Cleanup", True, "Successfully cleaned up test data"
                )
    ...
    async def _test_pytest_suite(self):
        ...
        try:
            ...
            critical_result = await self._run_pytest_file(
                "tests/test_critical_functionality.py"
            )
            self._record_test(
            ...
            # Run service tests
            service_result = await self._run_pytest_file(
                "tests/test_refactored_services.py"
            )
            self._record_test(
                "Refactored Services Tests",
            ...
            self._record_test(
                "API Routes Tests", api_result["success"], api_result["message"]
            )

            # Run utils tests
            utils_result = await self._run_pytest_file(
                "tests/utils/test_streamlink_utils.py"
            )
            self._record_test(
                "Streamlink Utils Tests",
                utils_result["success"],
                utils_result["message"],
            )

            # Run specific service tests
            recording_result = await self._run_pytest_file(
    ...
    async def _run_pytest_file(self, test_file: str) -> Dict[str, any]:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                error_summary = "\n".join(
                    [line for line in error_lines if line.strip()]
                )
                return {
                    "success": False,
                    "message": f"Tests failed: {error_summary[:200]}...",
                }

        except subprocess.TimeoutExpired:
            return {
                "success": False,
                "message": f"Tests timed out after 30s ({test_file})",
            }
        except Exception as e:
    ...
    async def _test_end_to_end_recording(self):
        ...
        try:
            ...
            self._record_test(
                "Test Video Creation", True, f"Created test video: {test_video_path}"
            )
        ...
        try:
            ...
            if result.returncode == 0 and os.path.exists(mp4_path):
                ...
                self._record_test(
                    "TS to MP4 Remux",
                    True,
                    f"Remuxed in {remux_time:.2f}s, output: {mp4_size} bytes",
                )
                ...
                error_output = (
                    result.stderr.decode() if result.stderr else "No error output"
                )
                self._record_test(
                    "TS to MP4 Remux", False, f"Remux failed: {error_output[:200]}"
                )
        ...
        try:
            ...
            if result.returncode == 0 and os.path.exists(output_path):
                self._record_test(
                    "Metadata Addition",
                    True,
                    f"Added {len(test_metadata)} metadata fields",
                )
                ...
                if verify_result.returncode == 0:
                    ...
                    found_tags = len(
                        [tag for tag in test_metadata.keys() if tag in tags]
                    )
                    self._record_test(
                        "Metadata Verification",
                        True,
                        f"Verified {found_tags}/{len(test_metadata)} metadata tags",
                    )

            else:
                self._record_test(
                    "Metadata Addition", False, "Failed to add metadata to MP4"
                )
    ...
    async def _test_video_processing_pipeline(self, test_video_path: str):
        ...
        try:
            ...
            if os.path.exists(test_video_path):
                self._record_test(
                    "Video File Detection", True, "Test video file exists"
                )
            else:
                self._record_test(
                    "Video File Detection", False, "Test video file missing"
                )
                return
            ...
            if os.path.exists(recordings_dir):
                self._record_test(
                    "Recording Directory", True, f"Directory: {recordings_dir}"
                )
    ...
    async def _test_streamlink_integration(self):
        ...
        try:
            ...
            if command and len(command) > 0:
                self._record_test(
                    "Streamlink Command Generation",
                    True,
                    f"Generated command with {len(command)} parts",
                )
            else:
                self._record_test(
                    "Streamlink Command Generation", False, "Failed to generate command"
                )
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    self._record_test(
                        "Streamlink Availability", True, f"Version: {version}"
                    )
                else:
                    self._record_test(
                        "Streamlink Availability", False, "Streamlink command failed"
                    )
            except (subprocess.TimeoutExpired, FileNotFoundError):
                self._record_test(
                    "Streamlink Availability", False, "Streamlink not found in PATH"
                )
    ...
    def _record_test(self, test_name: str, success: bool, message: str):
        ...
        # Also log to file
        self._log_test_detail(
            f"{'✅ PASS' if success else '❌ FAIL'} {test_name}: {message}"
        )
    ...
    def _print_test_summary(self):
        ...
        if failed_count == 0:
            ...
            logger.warning(
                f"⚠️  {failed_count} tests failed. Please fix issues before proceeding."
            )
