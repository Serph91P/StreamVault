...
class TaskProgressTracker:
    ...
    def __init__(self, websocket_manager=None, queue_manager=None):
        ...
        self.external_tasks: Dict[
            str, QueueTask
        ] = {}  # For external jobs like recordings
    ...
    def update_task_progress(self, task_id: str, progress: float):
        ...
        if hasattr(self, "_updating_progress") and task_id in self._updating_progress:
            logger.warning(
                f"Recursion detected in update_task_progress for task {task_id}, skipping"
            )
            return
        ...
        try:
            if task_id in self.active_tasks:
                ...
                logger.debug(
                    f"Task {task_id} progress: {old_progress:.1f}% -> {task.progress:.1f}%"
                )
                ...
                if abs(task.progress - old_progress) >= 5.0 or task.progress >= 100.0:
                    ...
                    self._create_background_task(
                        self._send_progress_update(task_id, task.progress)
                    )
        finally:
            self._updating_progress.discard(task_id)

    def register_progress_callback(
        self, task_id: str, callback: Callable[[float], None]
    ):
    ...
    def update_external_task_progress(self, task_id: str, progress: float):
        ...
        if task_id in self.external_tasks:
            ...
            logger.debug(
                f"External task {task_id} progress: {old_progress:.1f}% -> {task.progress:.1f}%"
            )
    ...
    def complete_external_task(self, task_id: str, success: bool = True):
        ...
        if task_id in self.external_tasks:
            ...
            logger.debug(
                f"External task {task_id} marked as {'completed' if success else 'failed'}"
            )
    ...
    async def _send_progress_update(self, task_id: str, progress: float):
        ...
        try:
            ...
            logger.warning(
                f"Failed to send WebSocket progress update for {task_id}: {e}"
            )
